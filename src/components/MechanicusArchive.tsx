...
const TAB_META: Record<Tab, { title: string; lede: string }> = {
  'Forge Worlds': {
    title: 'Forge Worlds of the Omnissiah',
    lede:  'Seven principal foundries whose god-engines and pattern-blades arm the Imperium. Select a forge to unseal its schematic.',
  },
  'Organisation': {
    title: 'Organisations of the Machine Cult',
    lede:  'The bodies, councils, and brotherhoods that govern the Adeptus Mechanicus across the galaxy.',
  },
  'Theology': {
    title: 'Theology of the Omnissiah',
    lede:  'The sacred tenets, divine aspects, and holy doctrine venerated by the Cult Mechanicus.',
  },
  'Rank': {
    title: 'Ranks of the Long Path',
    lede:  'From initiate Tech-Priest to Fabricator-General — the ordained hierarchy of the Machine God\'s servants.',
  },
  'Character': {
    title: 'Notable Servants of the Machine',
    lede:  'Individuals who have shaped the fate of the Adeptus Mechanicus — from legendary archmagos to arch-heretics.',
  },
  'Military': {
    title: 'Military Assets of the Forge Worlds',
    lede:  'The war-engines, cyborg legions, combat constructs, and sacred military institutions of the Omnissiah.',
  },
  'Technology': {
    title: 'Sacred Technologies',
    lede:  'Revered artefacts and ancient systems — the holy grails that the Mechanicus hunts across the stars.',
  },
  'History': {
    title: 'Historical Events',
    lede:  'Key episodes that defined the Machine Cult across the long millennia of the Imperium.',
  },
}
...
function entryFields(e: MechEntry): [string, string][] {
  ...
  if (e.founded)    pairs.push(['Founded',    e.founded])
  if (e.tier)       pairs.push(['Tier',       e.tier])
  if (e.period)     pairs.push(['Period',     e.period])
  if (e.homeworld)  pairs.push(['Homeworld',  e.homeworld])
  if (e.allegiance) pairs.push(['Allegiance', e.allegiance])
  if (e.aspect)     pairs.push(['Aspect',     e.aspect])
  if (e.worship)    pairs.push(['Worship',    e.worship])
  if (e.dogma)      pairs.push(['Dogma',      e.dogma])
  if (e.colors)     pairs.push(['Colors',     e.colors])
  if (e.strength)   pairs.push(['Strength',   e.strength])
  if (e.specialty)  pairs.push(['Specialty',  e.specialty])
  if (e.status)     pairs.push(['Status',     e.status])
  return pairs
}
  ...
  return (
    <div className="view mechanicus-view">
      <div className="mech-header">
        ...
        <h1>{meta.title}</h1>
        <p className="mech-header-lede">{meta.lede}</p>
      </div>
